...
const registerUser = async (req: Request, res: Response)=>{
    ...
    if(user) return res.status(409).json({status: false, message: "User already exist."});
    ...
    //end
    res.status(201).json({status: true, message: newCreatedUser});
}

const getAllUsers = async (req: Request, res: Response)=>{
    res.status(200).json({status: true, message: await getAllRegisteredUsers()});
}

const checkUser = async (req: Request, res: Response)=>{
    res.status(200).json({status: true, message: await getUserInfo((req as any).userId)});
}

const verifyUser = async (req: Request, res: Response)=>{
    const {email, token} = req.body;
    const user = await findUserByEmail(email)
    if(!user) return res.status(400).json({status: false, message: "No User with that email."});
    const foundToken = await checkToken(user.id, token);
    if(!foundToken) return res.status(400).json({status: false, message: "Invalid Token"});
    await verifyUserStatus(user.id)
    res.status(200).json({status: true, message: `User ${user.name} verified successfully.`});
}
...
const deleteUser = async (req: Request, res: Response)=>{
    ...
    if(deletedCount < 1) res.status(500).json({status: false, message: "Failed to delete User"});
    else res.status(200).json({status: true, message: "Deleted."});
}
...
const updateUser = async (req: Request, res: Response)=>{
    ...
    if(updatedUser.modifiedCount > 0) res.status(200).json({status: true, message: "User updated successfully"});
    else res.status(500).json({status: false, message: "Failed to update User"});
}
